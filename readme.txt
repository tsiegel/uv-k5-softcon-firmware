...
This makes it a bit confusing if you don't know what's going on, so be careful about choosing your step size. Also, note that the code multiplies your step size by 100 before applying it, so if you aren't careful, you might not get the step frequency you're after.
...
DCS/CTCSS transmit:
Both the receive and transmit CTCS values are represented here, which is why each one shows up twice.  First time is receive, second time is transmit, but the only option that speaks is the one saying "off".  The others are not accessible at this time.
...
up/down arrows short press move through the menus when in menu mode, and change your current frequency by the amount of your step value *100) when outside of menus.
